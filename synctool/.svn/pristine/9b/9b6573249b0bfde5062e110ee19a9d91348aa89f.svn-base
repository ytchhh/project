...
int nfsstart()
{
    string cmd("systemctl start nfs-kernel-service >/dev/null 2>&1");
...
int nfsstop()
{
    string cmd("systemctl stop nfs-kernel-service >/dev/null 2>&1");
    int ret = system(cmd.c_str());
    return ret;
...
int nfsstatus()
{
    string cmd("systemctl status nfs-kernel-service >/dev/null 2>&1");
    int ret = system(cmd.c_str());
	if(WIFEXITED(ret)) 
	{
